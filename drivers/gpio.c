...
// enables port clocks and sets the peripheral bus (vs the high speed bus)
void enablePort(PORT port)
{
    switch(port)
    {
        case PORTA:
            SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R0;	// port A clocks
            SYSCTL_GPIOHBCTL_R &= ~1;			// peripheral bus
            break;
        case PORTB:
...
// sets auxillary function (not push button or led) 
// gives it port & pin # & a function
// datasheet section 23.4 page 1351 and use the nums 0 thru 15 to set function of pin
// if function is less than 15, then shift function will shift to the left by num * 4
// else if u gave a mask value, ?
// then the swithc will have  a 32 bit number
// case statements erases the bits for the pin and replaces it with the new function\
// if the number/fn is greater than zero, then we turn on? the afsel bit
...
{
    uint32_t *p;
    p = (uint32_t *)port + pin + OFS_DATA_TO_IS;	// edge mode
    *p = 0;
    p = (uint32_t *)port + pin + OFS_DATA_TO_IBE;	// both edges = 0 to not both edge
    *p = 0;
    p = (uint32_t *)port + pin + OFS_DATA_TO_IEV;	// high level/ risign edge
    *p = 1;
}
...
{
    uint32_t *p;			
    p = (uint32_t *)port + pin;		// pointer to port register + pin 
    *p = value;				// write value to pin address
}

void togglePinValue(PORT port, uint8_t pin)
{
    uint32_t *p;
    p = (uint32_t *)port + pin;		// get the pin value
    *p ^= 1;				// toggle the pin value
}

bool getPinValue(PORT port, uint8_t pin)
{
    uint32_t *p;
    p = (uint32_t *)port + pin;		// get pin value/address
    return *p;
}
